%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 05-modelling.tex --- 正文层 (BODY LAYER)
%%
%% 这里只有内容。字体、边距、颜色、定理环境长什么样，全在
%% preamble.tex 里；改版式不用碰这个文件。
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\title{05 \midot{} Modelling}
\author{Harry Chen}
\date{}
\maketitle
...
\fig{split-vs-ladder}{One split against the walk-forward ladder. Above, a single timeline cut once
into train, validation and test. Below, the same history divided into twelve equal segments, with
the train--validation--test frame starting one segment later on each row---so each row's test
segment is the next row's validation segment.}{fig:split-vs-ladder}
...
\subsection{What the single split costs}\label{sec:split-costs}
...
\textbf{One fit discards time}
  & A single set of coefficients over several years asserts that those years were one market.
    Early in the pandemic the world locked down at once and nobody wanted oil; the consensus was
    that demand had permanently moved online. It had not---the economy reopened, and energy then
    flew on war. A coefficient estimated across both states describes neither. \\ \hline
\textbf{Almost nothing is left to score}
  & Give validation a year and test a year and validation is roughly 250 dates. A year is also too
    little market: the regimes it contains are whichever ones happened to fall inside it, so the
    split never asks what the strategy does when the regime turns. A maximum drawdown measured
...
\textbf{The sample is small and the signal is faint}
  & A school dataset with a thousand points and an $R^2$ of 0.5 is ordinary, and in physics 0.9 is
    ordinary. In finance \textbf{0.1 to 0.2 is already a good $R^2$}---see
    \chapref{09}{09-ic-and-r-squared}. Weak true dependence plus few observations is precisely the
    regime in which a fit lands on noise: a line through ten points drawn from a cloud reaches
    $R^2$ 0.8 easily. \\ \hline
\end{tabularx}

\subsection{The fix that is not one}\label{sec:not-a-fix}

\begin{claim}\label{clm:duplication}
Duplicating the sample changes no coefficient, and inflates every t-statistic by roughly
$\sqrt{2}$.
\end{claim}

\begin{proof}
Write the fit as $b = (X^{T} X)^{-1} X^{T} y$, with $X$ the $n \times p$ matrix of signal values,
$y$ the forward returns, and $b$ the estimated coefficients. Stacking the sample twice gives
$X_2^{T} X_2 = 2\, X^{T} X$ and $X_2^{T} y_2 = 2\, X^{T} y$, so
\[
...
---halved. Standard errors fall by $\sqrt{2}$ and t-statistics rise by the same factor, on
...
more of the ones you have, which is \secref{sec:ladder}.
\end{note}
...
\chapref{08 \midot{} Toolbox}{08-toolbox-pandas}.}
...
$X$, $y$, $b$, $n$, $p$, $s^2$
  & the design matrix, the response, the estimated coefficients, the number of dates, the number
    of signals, the residual variance estimate
  & \secref{sec:not-a-fix} \\ \hline
$\lambda$
...
\chapref{02 \midot{} \S 4}{02-testing-a-signal}'s $\sigma_{s,t}$, an asset's trailing volatility,
and not \chapref{04 \midot{} \S 1.1}{04-volatility-regimes}'s $\sigma$, the amplitude of the tape.
$\beta_j$ is a coefficient on a \textbf{signal}, as in
\chapref{04 \midot{} \S 3.2}{04-volatility-regimes}, never a market beta. $\lambda$ is the penalty
...
\chapref{04 \midot{} \S 2.1}{04-volatility-regimes}, here over a signal rather than over returns.
\end{note}
...
\begin{xltabular}{\linewidth}{|>{\raggedright\arraybackslash}p{0.36\linewidth}|X|}
\hline
\textbf{Belief} & \textbf{Correction} \\ \hline
\endhead
``More rows will fix the small sample.''
  & Duplicating them changes no coefficient and shrinks the standard errors by $\sqrt{2}$
    ---Claim~\ref{clm:duplication}. Score more of the dates you have instead. \\ \hline
...
``Clip against the series I am building.''
  & The band must come from the raw history, or it walks inward on itself
    ---\secref{sec:clipping}. \\ \hline
``Standardizing with the whole training window is look-ahead.''
  & The model is constructed at the end of that window, so the window is history to it. Using the
    \emph{validation} segment is the leak---\secref{sec:which-stats}. \\ \hline
``The validation block is a second test set.''
  & It chooses the model class, $\lambda$, $\gamma$ and the cap. Everything it chooses on, it has
    been fitted to. \\ \hline
``A bigger $\beta$ means a more important signal.''
  & Only after \secref{sec:standardizing}. Before it, $\beta$ is a statement about units. \\ \hline
\end{xltabular}
...
\addcontentsline{toc}{section}{Next $\rightarrow$ Backtest Prototype}
...
\secref{sec:standardizing}'s figure drawn from your own numbers rather than from an illustration.
...
\pointer{The lecture this chapter is drawn from, including the market history behind it:
\href{../../market_knowledge/walk-forward-modelling.md}{Walk-Forward Modelling, Winsorization, and
Standardization}.}

You should be able to explain:

\begin{checklist}
  \checkitem Why a single split ignores time, and why one year of validation is not a risk estimate
  \checkitem What duplicating the sample does to the coefficients and to their standard errors
  \checkitem Why each rung's test segment is the next rung's validation segment
  \checkitem Which decisions belong to the validation segment, which to the held-out block, and
             which to neither
  \checkitem Why an outlier is clipped rather than deleted, and why the band is computed from the
             raw series
  \checkitem Why coefficients are meaningless across signals of different scale, and what the
             coefficient paths show once they are not
\end{checklist}

\vspace{1em}
\noindent\chapref{$\leftarrow$ 04}{04-volatility-regimes} \midot{}
\chapref{How a Strategy Is Built}{00-pipeline} \midot{} reference:
\chapref{08 \midot{} Toolbox}{08-toolbox-pandas}
