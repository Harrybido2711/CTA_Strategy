...
> - **Answers:** how several signals are combined into one prediction without the fit quietly learning the answer — how the sample is split, which split chooses what, and what has to be done to a signal before a model may see it.
> - **Prerequisites:** [02 · Testing a Signal](02-testing-a-signal.md) § 3.2 for fitting one signal against forward returns; [04 · Volatility Regimes](04-volatility-regimes.md) § 3.2 for regressing one signal on the others; [09 · IC and R²](09-ic-and-r-squared.md) for the scale of R² this chapter takes for granted.
> - **After reading:** say why a single train/validation/test split is not a backtest, how a walk-forward ladder is built and what each of its two validation layers is allowed to decide, why an outlier is clipped rather than deleted, why signals must be put on one scale before their coefficients mean anything, and which mean and standard deviation you may legally use at each step.

---

Up to here every signal has been studied alone: does it order forward returns, and does it add
anything to the signals already in the book. Combining them is a **fit**, and a fit is where a
research pipeline usually starts lying to you. Three steps, in order. **One split is one fit, and one
fit is not a backtest** (§ 1). **The ladder replaces it, and its two validation layers answer
different questions** (§§ 2–3). **A signal is clipped and rescaled before the model is allowed to
see it** (§ 4).
...
| **The sample is small and the signal is faint** | A school dataset with a thousand points and an R² of 0.5 is ordinary, and in physics 0.9 is ordinary. In finance **0.1 to 0.2 is already a good R²** — see [09](09-ic-and-r-squared.md). Weak true dependence plus few observations is precisely the regime in which a fit lands on noise: a line through ten points drawn from a cloud reaches R² 0.8 easily. |
...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="figures/outer-validation-dark.png">
  <img alt="A timeline of train, validation and test, with the validation block ringed in violet. Three miniature ladder rows sit inside the training block, labelled as the inner layer that chooses which features survive. An arrow from the ringed block leads to a list: the model class, its hyperparameters, and the optimizer's parameters" src="figures/outer-validation-light.png">
</picture>
...
→ `rolling`, `quantile` and `clip` as pandas calls: [08 · Toolbox](08-toolbox-pandas.md).
...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="figures/beta-paths-dark.png">
  <img alt="Three coefficient paths on one axis over a year of refits: momentum starts high and decays toward zero, a volatility-regime signal rises from zero to become the largest, and MACD stays in between. Two shaded stretches mark where momentum carries the book and where the lean rotates to the regime signal" src="figures/beta-paths-light.png">
</picture>
...
rescale it — not [02 § 4](02-testing-a-signal.md)'s $\sigma_{s,t}$, an asset's trailing
volatility, and not [04 § 1.1](04-volatility-regimes.md)'s $\sigma$, the amplitude of the tape.
$\beta_j$ is a coefficient on a **signal**, as in [04 § 3.2](04-volatility-regimes.md), never a
...
[04 § 2.1](04-volatility-regimes.md), here over a signal rather than over returns.
...
[← 04](04-volatility-regimes.md) · [How a Strategy Is Built](00-pipeline.md) · reference: [08 · Toolbox](08-toolbox-pandas.md)
