"""Figures for docs/03_shaping_the_lookback.tex.
...
def fast_times_the_slow(mode):
    """03 s2 -- two windows in, one signal out.

    Schematic. One price path that falls, turns and rolls over, with a fast and
    a slow moving average drawn over it. The lower panel is the single series
    the pair produces -- the fast average minus the slow one -- so its zero
    crossings are exactly the two points where the lines above cross, and the
    rule is the sign of that one number.
    """
    t = THEMES[mode]
    ...
    # The pair produces one series, not two rules run side by side.
    signal = fast_ma - slow_ma

    def cross(lo, hi, up):
        for i in range(lo + 1, hi):
            if (signal[i] > 0 >= signal[i - 1]) if up else (signal[i] < 0 <= signal[i - 1]):
                return i
        return None

    peak = int(np.nanargmax(level))
    enter, leave = cross(N_S, peak, True), cross(peak, n, False)
    day = np.arange(n)
    ...
    # ---- top: the price, and the two averages whose gap is the signal
    style_axes(ax, t, ylabel="price, rebased to 100")
    lo, hi = 88, 128
    ax.set_ylim(lo, hi)
    ax.set_xlim(0, n)
    for i in (enter, leave):
        ax.plot([i, i], [lo, hi], color=t["ink_secondary"], linewidth=1.0,
    ...
    ax.plot([enter, leave], [90.6, 90.6], color=t["ramp"][3], linewidth=3.6,
            solid_capstyle="butt", zorder=5)
    ax.text(enter - 4, 90.6, "long, on this one signal", color=t["ink_secondary"],
            fontsize=8.3, fontweight="600", ha="right", va="center")

    titles(ax, t, "Two windows in, one signal out",
           "illustrative — the pair trades the gap between the averages, not each window on its own")

    # ---- bottom: that gap, as a single series crossing zero
    style_axes(bx, t, ylabel="signal:  fast − slow", xlabel="trading day")
    ylo, yhi = -3.6, 6.4
    bx.set_ylim(ylo, yhi)
    bx.set_xlim(0, n)
    for i in (enter, leave):
        bx.plot([i, i], [ylo, yhi], color=t["ink_secondary"], linewidth=1.0,
                linestyle=(0, (2, 2.4)), zorder=1)
    bx.fill_between(day, 0, signal, where=signal > 0, interpolate=True,
                    color=t["ramp"][1], alpha=0.5, linewidth=0, zorder=2)
    bx.axhline(0, color=t["baseline"], linewidth=1.0, zorder=3)
    bx.plot(signal, color=t["ramp"][3], linewidth=2.2, zorder=4)

    for i in (enter, leave):
        bx.plot([i], [0], marker="o", markersize=5.4, color=t["ramp"][3],
                markeredgecolor=t["surface"], markeredgewidth=1.2, zorder=6)

    bx.text(enter + 5, 5.5, "fast overtakes slow\n— the signal turns positive",
            color=t["ink"], fontsize=8.4, fontweight="600", ha="left", va="top",
            linespacing=1.45)
    bx.text(leave - 5, 5.5, "and falls back under\n— it turns negative",
            color=t["ink"], fontsize=8.4, fontweight="600", ha="right", va="top",
            linespacing=1.45)
    bx.text((enter + leave) / 2, 0.55, "signal > 0 — held long",
            color=t["ink_secondary"], fontsize=8.2, ha="center", va="bottom")

    fig.text(0.5, -0.005,
             "The signal is the fast average minus the slow one, and the rule is its sign. It crosses "
             "zero exactly\nwhere the two lines above cross, so the pair is one indicator rather than "
...
if __name__ == "__main__":
    for mode in ("light", "dark"):
        for f in FIGURES:
            f(mode)
    print(f"wrote {2 * len(FIGURES)} figures for 03_shaping_the_lookback")
