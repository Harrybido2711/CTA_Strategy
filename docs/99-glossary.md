...
| Bucketed bar chart | 分组柱状图 | Sort by signal, split into groups, plot mean forward return per group — the core signal test | [02](02-testing-a-signal.md) |
| Reversal | 反转 | Extreme recent moves partially undo themselves; strongest at high frequency | [02](02-testing-a-signal.md) |
| Skip / lag | 跳过期 | Dropping the most recent period from the lookback so reversal does not contaminate momentum | [02](02-testing-a-signal.md) |
| EWMA | 指数加权移动平均 | Moving average weighting recent observations more heavily; tuned by half-life | [03](03-shaping-the-lookback.md) |
| Half-life | 半衰期 | Periods after which an EWMA observation's weight has decayed by half | [03](03-shaping-the-lookback.md) |
| Volatility clustering | 波动率聚类 | Volatility arrives in bursts, not evenly — what makes a fast signal churn | [02](02-testing-a-signal.md) |
| Smoothing | 平滑 | Filtering the shortest cycles out of a fast signal; window must be shorter than its period | [02](02-testing-a-signal.md) |
...
| Risk-adjusted momentum | 风险调整动量 | `Avg(r/σ)` — momentum divided by volatility so values compare across assets and regimes | [02](02-testing-a-signal.md) |
| Rolling-rank bucketing | 滚动历史排序分组 | Rank a signal only against history strictly before `t`, avoiding look-ahead | [02](02-testing-a-signal.md) |
| Error bar | 误差棒 | Uncertainty of a bucket's mean; what distinguishes a real bar from a 2-sample one | [02](02-testing-a-signal.md) |
