...
%% Page formatting.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% A textbook sets 60-75 characters to the line; 1in margins on letter give a
% 6.5in measure, which is about 90 and long enough that the eye loses the
% return. 6.0in is about 83 -- still long, but 5.6in (77, the textbook figure)
% left more white on the page than the shorter line was buying back. The width
% goes to the margin, not to the type: the body stays 12pt.
\usepackage[top=0.95in, bottom=1.05in, left=1.25in, right=1.25in,
            headheight=15pt, headsep=0.25in]{geometry}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% A running head is what tells a reader mid-chapter which chapter they are in
% -- the one piece of furniture every textbook has and lecture notes do not.
% article's own "myheadings" style already sets it (mark left, folio right),
% so no package is needed; \chapterhead fills the mark.
\pagestyle{myheadings}

% The fourth level was \normalsize\bfseries -- the same size and weight as the
% run-in \textbf{...} labels the body uses inside a paragraph, so a heading and
% an emphasis were indistinguishable. Italic separates them.
...
% sections print as "5.1", subsections as "5.1.1", and the shared theorem
% counter restarts too, so Definition 3 of chapter 5 reads "Definition 5.3"
% and cannot collide with chapter 2's. A section number therefore means the
% same thing in the standalone PDF and in the bound edition, and \secref
% prints "\S 5.4.3" in both.
%
% #1 is written the way the folder writes it -- \chapterhead{05}{...} -- and
% {\number#1} strips the leading zero at the point of printing, so the body
% still names its own folder and "05" never reaches the page. The braces are
% load-bearing: \number swallows a space that terminates it, so
% "\number#1 \midot" would set "5-dot" with no space at all.
\newcommand{\chapterhead}[3]{%
  \ifbook
    ...
    \thispagestyle{plain}%
    {\LARGE\bfseries {\number#1} \midot{} #2\par}%
    \vspace{0.35em}{\color{rulecol}\rule{\linewidth}{0.8pt}}\par\vspace{0.5em}%
  \else
    \markright{{\number#1} \midot{} #2}%
    \title{{\number#1} \midot{} #2}%
    \author{Harry Chen}%
  ...
  \renewcommand{\thesection}{\number#1.\arabic{section}}%
  \setcounter{definition}{0}%
  \renewcommand{\thedefinition}{\number#1.\arabic{definition}}%
  \setcounter{figure}{0}%
  \renewcommand{\thefigure}{\number#1.\arabic{figure}}%
}
...
% Interpunct, for titles like "5 · Modelling".
...
% \chapterhead prints "5.1" and "5.4.3" -- and "100.4.3" once the dataset
% chapter converts -- which overrun them and collide with the title text, so
% each column is widened and the entries re-indented.
\makeatletter
\renewcommand*\l@section{\@dottedtocline{1}{1.5em}{2.6em}}
\renewcommand*\l@subsection{\@dottedtocline{2}{4.1em}{3.4em}}
\renewcommand*\l@subsubsection{\@dottedtocline{3}{7.5em}{4.2em}}
\makeatother
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
