...
\subsection{The method, and why it works}\label{sec:slide}

The three defects of \secref{sec:split-costs} share one cause: the model is fitted once. So fit it
many times. Cut the training history into short segments, put a train--validation--test frame on the
first few of them, and slide that frame forward one segment per iteration until it reaches the end
of the training set.
...
The ruler stops where the training set ends. The held-out block beyond it never enters the ladder,
and what it alone may decide is \secref{sec:two-layers}.

Number the segments $1, 2, 3, \ldots$ and the frame moves like this:
...
\subsection{Two validation layers}\label{sec:two-layers}
...
\textbf{Why the inner layer cannot be skipped.} Exploration already looked at which signals worked
\emph{in the training period}. Of course the fit then describes that period well: momentum was selected partly because it was seen to work
...
\textbf{What only the outer block may decide.} $\lambda$ is not fitted, it is searched: run the
ladder once per value on a grid---0.1, 0.5, 0.7, 1---and compare. The same is true of the model class itself (Lasso, Ridge, a decision tree). Both
comparisons need dates that no rung of the ladder consumed, which is exactly what the held-out
block is.

\textbf{Side by side, as objectives.} The inner layer solves the penalized fit of
\secref{sec:one-rung} with $\lambda$ handed to it,
\[
  \min_{\beta} \quad \sum_t \left( y_t - \sum_j \beta_j x_{j,t} \right)^{2}
    + \lambda \sum_j \left| \beta_j \right|,
\]
so the only things it may move are the coefficients $\beta_j$ and, through the penalty, the set of
signals that survive with $\beta_j \neq 0$.

The outer layer's objective is the book itself, because the prediction is not yet a portfolio.
Weights come from a mean--variance problem: choose $w$ to
\[
