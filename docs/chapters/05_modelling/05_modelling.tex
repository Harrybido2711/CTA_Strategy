...
\textbf{The sample is small and the signal is faint}
  & A school dataset with a thousand points and an $R^2$ of 0.5 is ordinary, and in physics 0.9 is
    ...
    \chapref{9}{09_ic_and_r_squared}. Weak true dependence plus few observations is precisely the
...
\chapref{2 \midot{} \S 4}{02_testing_a_signal}'s $\sigma_{s,t}$, an asset's trailing volatility,
and not \chapref{4 \midot{} \S 1.1}{04_volatility_regimes}'s $\sigma$, the amplitude of the tape.
$\beta_j$ is a coefficient on a \textbf{signal}, as in
\chapref{4 \midot{} \S 3.2}{04_volatility_regimes}, never a market beta. $\lambda$ is the penalty
...
\chapref{4 \midot{} \S 2.1}{04_volatility_regimes}, here over a signal rather than over returns.
\end{note}
...
\vspace{1em}
\noindent\chapref{$\leftarrow$ 4}{04_volatility_regimes} \midot{}
