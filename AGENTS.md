...
| `.claude/scripts/check_sync.py` | The consistency check and the `--impact` work list. Local only |
| `.githooks/pre-commit` | Runs that check at every commit. Committed; enable with `git config core.hooksPath .githooks` |
...
| **T9** | Synchronize — after any edit, and before any move, rename or delete | [`.claude/tasks/synchronization.md`](.claude/tasks/synchronization.md) | `simplify` | `check_sync.py` is clean, or every finding is declared with a reason |
...
## 4. The closing pass — every task ends here

No task is finished at its own "Done when". **Every one of them then runs this**, which is why it
is written once here instead of in twelve rows. Synchronising is not something big changes get and
small ones skip; it is how the repository stays coherent.

**Sync per finished change, not per batch of them.** Two files disagree because one was edited and
the other, which describes it, was not — and the commit is the last cheap moment to notice.

```bash
python3 .claude/scripts/check_sync.py --impact <what you are about to change>   # before editing
./docs/build.sh <stem> && ./docs/build.sh --book                                # if a .tex or figure moved
python3 .claude/scripts/check_sync.py                                           # after editing
git add <the files you touched, by name>
git commit -m "<what changed conceptually>"
git push origin main
```

1. **Before editing**, `--impact` gives the work list — every file that mentions what you are
   changing. Change them together, or say in the commit why one was left.
2. **Rebuild** if the task touched a `.tex`, a figure or `preamble.tex`. A correct chapter PDF
   beside a stale `all_chapters.pdf` is the failure that survives every other check.
3. **Check**, and fix each finding or declare it in `.claude/sync-exceptions.json` **with a
   reason**. A finding that is neither is an error.
4. **Commit and push** to `origin main` without being asked. One commit per coherent unit of work.
   **Stage the files you touched by name — never `git add -A`**, which has already swept unrelated
   work into a commit here. The message says what changed conceptually, not "update files".

`.githooks/pre-commit` runs the check and the drift report at every commit (`git config
core.hooksPath .githooks` once per clone). It reports and does not block, deliberately, while
chapters are still in Markdown — a gate that fires daily is a gate that gets bypassed.

**State which layers you ran and why any was skipped.** An unstated skip cannot be told apart from
having forgotten. The three layers, and what each finding means:
[`.claude/tasks/synchronization.md`](.claude/tasks/synchronization.md).
