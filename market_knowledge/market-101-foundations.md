...
### 1.2 The core logic: one risk factor per class

**An asset class is a set of instruments driven by the same risk factor.** That is the
whole of it. Every other criterion is a stand-in for this one, used because the risk
factor is not printed on the instrument.

**Why that, and not "different real-life uses"?** Because a classification only earns its
keep if it **predicts co-movement**: things inside a class move together, things across
classes need not. Sorting by real-life use fails that test — gold has industrial uses and
is also a monetary hedge, and the industrial use tells you nothing about how it trades.
...
The observable criteria, and what each is really proxying for:

| Criterion | Proxies the risk factor because… |
| --- | --- |
| **Claim type** | What you own determines what can go wrong with it — the causal root |
| **Carry** | The cash flow is generated by the same mechanism the risk factor moves |
| **Settlement** | Physical delivery ties the price to a physical supply chain (§ 1.3) |
| **Venue and liquidity** | Where it trades bounds how fast the factor gets repriced (§ 5) |

#### A second, independent question: can the class pay anyone?
...
the equity rallies; a REIT is real estate by claim and a listed equity by venue. The
classification is a tool for reasoning about risk, not a partition of the world — when an
instrument straddles two rows, **classify it by the risk factor that dominates its P&L.**
