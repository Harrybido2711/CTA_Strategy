...
Read it as a pipeline — $P$ eats a member of $F$ and returns a number:

| Input: an event $E \in F$ | Output:$P(E) \in [0,1]$ |
| ------------------------------------------------------------ | ------- |
| $\emptyset$ — nothing happens                             | $0$   |
| $\{3\}$ — rolled a 3                                      | $1/6$ |
| $\{2, 4, 6\}$ — rolled even                               | $1/2$ |
| $\Omega$ — something happened                             | $1$   |

Each left-hand entry is on the list $F$, and **that membership is what makes the
right-hand entry exist at all**. Shrink $F$ and rows drop out: if all you observe is
parity, $\{3\}$ is no longer an event and its row has no right-hand side.
...
Roll a die for money — $\Omega = \{1, \ldots, 6\}$ and $X(\omega)$ is **the payout**:

```math
X(\omega) = \begin{cases} 10 & \omega \in \{2, 4, 6\} \\ -5 & \omega \in \{1, 3, 5\} \end{cases}
```

Then $\{X \leq 0\}$ pulls back to the subset $\{1, 3, 5\}$, and it is that **set**, never
the number, that $P$ is handed: $P(X \leq 0) = P(\{1, 3, 5\}) = 1/2$.

#### 2.1.1 Discrete

$X$ is **discrete** when a finite or countably infinite set $S$ carries all the
probability — that is, $P(X \in S) = 1$. A die roll is the finite case; the number of
heads in the St Petersburg paradox is the countably infinite one.

#### 2.1.2 Continuous

$X$ is **continuous** when a nonnegative function $f$ — the density — exists with

$$
P(a \leq X \leq b) = \int_a^b f(x) dx \qquad \text{for all } a < b
$$

**Note (a precision usually skipped).** The loose version is $P(X = x) = 0$ for every $x$,
since any interval holds uncountably many outcomes. That is **necessary but not
sufficient**; the exact term is *absolutely continuous*, and graduate texts insist on it
because the analysis breaks without it. For interview purposes the loose version is fine.

#### 2.1.3 Neither

**A random variable need not be one or the other**, and people forget it. Flip a coin: on
heads draw from Uniform(0,1), on tails return $-1$. The result carries a point mass at
$-1$ *and* a density on $[0,1]$ — a mixture, so neither a PMF nor a PDF describes it
alone.
...
| **PDF** $p_X(x)$ | The derivative of the CDF | Continuous $X$ only |
| **PMF** $p_X(x)$ | $P(X = x)$ | Discrete $X$ only |

#### 2.2.1 CDF — always exists

$F_X(x) = P(X \leq x)$. It is defined for every random variable, because every random
variable can be asked whether it came in below a threshold. Four properties follow
**immediately from that definition** rather than being extra assumptions:
...
#### 2.2.2 PDF — continuous only

$p_X$ is the **derivative of the CDF**, so it exists only where the CDF is differentiable.
For our purposes: **a PDF exists if and only if $X$ is continuous** — absolute continuity
(§ 2.1.2) is what makes that precise.

#### 2.2.3 PMF — discrete only

$p_X(x) = P(X = x)$, the "point probabilities". Apply the same definition to a continuous
